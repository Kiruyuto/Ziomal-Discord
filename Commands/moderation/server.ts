  ...
  callback: async ({ client, message, interaction: slashCmd }) => {
    try {
      ...
      const guildInfo = new DCJS.MessageEmbed({
        title: `${guild.name}`,
        description: `${guild.name} was created on ${`<t:${Math.round(
          new Date(guild.createdTimestamp).getTime() / 1000
        )}:F>`}`,
        color: `${colorValues.embedDefault}`,
        thumbnail: {
          url:
            `${guild.iconURL({ dynamic: true })}` ??
            "https://i.imgur.com/0ABYGXT.png",
        },
        fields: [
          {
            name: "Total Members",
          ...
          },
          {
            name: "Total Humans",
            value: `${
              guild.members.cache.filter((members) => !members.user.bot).size
            }`,
            inline: true,
          },
          {
      ...
      let embedDev = new DCJS.MessageEmbed({
        description: `:x: Something went wrong with \`server\` command in the **${
