  ...
  callback: async ({ client, message, interaction: slashCmd }) => {
    try {
      const channel = message
        ...
        : (slashCmd.member as GuildMember).voice.channel;
      ...
      if (
        ...
        return new DCJS.MessageEmbed({
          color: `${colorValues.embedDefault}`,
          description:
            ":warning: You can't use it when im already in the channel!",
        });
      }
      ...
      let embedDev = new DCJS.MessageEmbed({
        description: `:x: Something went wrong with \`join\` command in the **${
