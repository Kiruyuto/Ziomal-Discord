...
export default {
  ...
  minArgs: 1,
  expectedArgs: "<URL> or <NAME TO SEARCH>",
  ...
  callback: async ({ client, message, args, interaction: slashCmd }) => {
    try {
      const working = message
        ...
        : slashCmd.channel?.send("Working on it...");
      if (!working) {
        return;
      }
      ...
      const embedPlaying = new DCJS.MessageEmbed({
        author: {
          ...
          iconURL: `${
            message
              ...
              : (slashCmd.member as GuildMember).user.displayAvatarURL()
          }`,
        },
        ...
        fields: [
          {
            name: "Duration",
            value: `${queue?.songs[queueLength].formattedDuration}`,
            inline: true,
          },
          {
            name: "Queue position",
...

          {
            name: "Added by",
            value: `<\@${
              message ? message.author.id : (slashCmd.member as GuildMember).id
            }>`,
            inline: true,
          },
          {
            name: "Repeat mode",
            value: `${loopState[queue?.repeatMode!]}`,
            inline: true,
          },
          {
            name: "Link",
            value: `[Click!](${queue?.songs[queueLength]?.url})`,
            inline: true,
          },
        ],
      });
      ...
      let embedDev = new DCJS.MessageEmbed({
        description: `:x: Something went wrong with \`play\` command in the **${
