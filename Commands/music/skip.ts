  ...
  callback: async ({ client, message, interaction: slashCmd }) => {
    try {
      ...
      let embedDev = new DCJS.MessageEmbed({
        description: `:x: Something went wrong with \`skip\` command in the **${(message ? message.guild?.name : slashCmd.guild?.name)}** \`(${message ? message.guild?.id : slashCmd.guild?.id})\` sever!`,
        color: `${colorValues.embedDefault}`,
      })
