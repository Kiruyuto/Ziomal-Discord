...
client.on("ready", () => {
  ...
  distube.on("error", (channel, error) => {
    ...
    client.users.fetch(IDs.KIRU).then((user) => {
      user.send({ content: `${error}`, embeds: [x] });
    });
    let ChannelEmbed = new DCJS.MessageEmbed({
      description:
        ":x: An error with music player occured! Message has been sent to developer.",
    });
